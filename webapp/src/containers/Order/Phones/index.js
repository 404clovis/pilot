    ...
    </span>
    {
      props.phones.map(phone => (
        <span className={style.phonesItem}>
          ...
          <a className={style.facebookIcon} href={`https://www.facebook.com/search/people/?q=${phone.area_code}${phone.number}`} target="_blank" >
            <FacebookIcon />
          </a>
          <a className={style.skypeIcon} href={`tel:0${phone.area_code}${phone.number}`}>
            <FaPhoneSquare />
          </a>
        </span>
      ))
